<template>
    <div>
        ...
        <div v-if="!isLoggedIn" class="flex justify-center items-center p-4 bg-blue-50 rounded-lg mb-4">
            <span>There are no proirities. You should login. </span>
...
try {
    await store.dispatch('proirity/deleteProirity',id)
...
const Data = computed(() => store.state.proirity.proirities);
const store = useStore()
// Lifecycle
onMounted(async () => {
    try {
        await store.dispatch('proirity/fetchProirities');
    } catch (error) {
        console.log(error)
    }
});
