    ...
    <div class="card flex justify-center">
        <Dialog v-model:visible="localVisible" modal header="Add" :style="{ width: '25rem' }">
            ...
            <div class=" items-center gap-4 mb-8">
                ...
                <InputText id="description" class="flex-auto w-full my-2" :value="formDesciption.en" v-model="formDesciption.en"
                    autocomplete="off" />
            </div>
            <div class=" items-center gap-4 mb-8">
...
const data = computed(() => {
    const task = store.state.task.task
    return task || null;
});
...
onMounted(async () => {
    try {
        if (props.id) {
            ...
            await store.dispatch('task/showTask', props.id);
