        ...
        <Dialog v-model:visible="localVisible" modal header="Show" :style="{ width: '25rem' }">
            <template v-if="data">
                ...
                <div class="flex items-center gap-4 mb-8">
                    <label for="description" class="font-semibold w-24">Email:</label>
                    <span>{{ data.email }}</span>
                </div>
                <div class="flex items-center gap-4 mb-8">
                    <label for="description" class="font-semibold w-24">Email:</label>
                    <span v-for="team in data.teams">{{ team.name }}</span>
                </div>
...
const data = computed(() => {
    const task = store.getters['task/getTaskById'](props.id)
    return task || null;
})
